...
func reset_game_state() -> void:
	...
	reset_player_state()
	var rip_count_node = get_node_or_null("/root/rip_count")
	if rip_count_node:
		rip_count_node.depression_rip_count = 0
...
func handle_movement(delta):
	var veloc = get_input_velocity()
	update_position(veloc, delta)
	update_animation(veloc)

func get_input_velocity():
	var input = Input.get_vector("move_left", "move_right", "move_up", "move_down")
	return input.normalized() * speed if input.length() > 0 else Vector2.ZERO

func update_position(_velocity, _delta):
...
func update_animation(_velocity):
	if _velocity.length_squared() > 0:
		if abs(_velocity.x) > abs(_velocity.y):
			$AnimatedSprite2D.play("walk_right" if _velocity.x > 0 else "walk_left")
		else:
			$AnimatedSprite2D.play("walk_forward" if _velocity.y > 0 else "walk_back")
	else:
...
func add_energy(amount: int):
	PlayerState.energy = min(PlayerState.energy + amount, MAX_ENERGY)
	energy = PlayerState.energy
	update_energy_display()
